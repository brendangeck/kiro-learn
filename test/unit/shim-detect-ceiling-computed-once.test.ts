 ...
 *
 * This test mocks `realpathSync` and `homedir` and asserts:
 *
 ...
 * `shim-detect-realpath-home-throws.test.ts`.
 *
...
describe('detectProjectRoot — ceiling computed once (Requirement 2.1)', () => {
  ...
  it('calls realpathSync(homedir()) at most once per invocation', () => {
    ...
    // Mocked homedir() returns a deterministic value with no OS or env
    // interaction.
