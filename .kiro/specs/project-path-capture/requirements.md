...
4. THE updated shim SHALL NOT require a collector that has applied Migration_0003. If the updated shim runs against a pre-0003 collector, `source.project_path` SHALL round-trip via the `source_json` column — the collector serialises the full `source` object with `JSON.stringify` on write and deserialises it with `JSON.parse` on read, so every own property of `source` is preserved even if the collector has no dedicated column for it. The only capability a pre-0003 collector lacks is the denormalised `events.project_path` column for indexed aggregation, which no v1 read path uses. *(Stretch guarantee; the intended deployment order is collector first, then shim, but this is not enforced.)*
