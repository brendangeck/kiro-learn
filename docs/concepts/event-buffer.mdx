...
The buffer watcher monitors each project's buffer and fires an [extraction](/architecture/extraction) trigger when either condition is met:
...
flowchart TD
    ...
    B -->|Yes| C[Fire extraction]
    B -->|No| D[Reset idle timer to 5s]
    D --> E{Timer expires?}
    E -->|Yes| C
    E -->|No| F[Wait for next event]
```

The idle timer ensures that even a single event gets extracted eventually — you don't need to fill the buffer to trigger processing. The size threshold ensures that bursts of activity are batched efficiently rather than extracted one-by-one.

If extraction is already in-flight for a project, additional triggers are suppressed until the current extraction completes.
