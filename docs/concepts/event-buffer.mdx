...
The buffer watcher monitors each project's buffer and fires an [ingestion trigger](/architecture/ingestion) when either condition is met:
...
The 30-second default sits in the middle of the configurable range. Shorter intervals make memory feel more responsive but give the two-stage [Ingestion Pipeline](/architecture/ingestion) fewer candidates to cluster against each other. Longer intervals let more context accumulate per batch, which helps reconciliation find duplicates, at the cost of more latency between "something interesting happened" and "it's searchable". Tune via `bufferIdleMs` on `CollectorConfig` — values outside `[5_000, 300_000]` milliseconds are rejected at daemon startup.
