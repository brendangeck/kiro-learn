...
The vector stage reads its data from the database exactly once per namespace via `listEmbeddings`, then keeps a normalized, in-process cache. Subsequent searches in the same namespace skip the database for the vector side and score directly against the cache; cache misses and invalidations are covered in "How search works" below.
