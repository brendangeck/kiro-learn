...
<Note>
  Retrieval reads `memory_records` unchanged from today. When the [Reconciliation Stage](/architecture/ingestion#reconciliation-stage) commits a merge, the merged originals are deleted from `memory_records` in the same transaction — see [`deleteMemoryRecord` cascade semantics](/architecture/database#merge-deletion-is-destructive) for details. There is no "merged" flag to filter on; merged rows are simply not in the table, so FTS5, the vector cache, and the fused hybrid ranking all exclude them by construction.
</Note>
