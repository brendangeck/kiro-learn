...
<Warning>
  **Merge commits have no undo.** A `<merge>` decision deletes the original rows outright. Nothing in the database remembers that `record_id mr_abc123` used to exist, and nothing in the retrieval path can surface a merged-away record. Tune the [extraction worker's thresholds](/architecture/extraction#configuration) conservatively — a low intra-batch or neighbor similarity threshold paired with a lenient judge can silently collapse rows that should have stayed distinct, and the only recovery path is to re-learn from fresh events.
</Warning>
