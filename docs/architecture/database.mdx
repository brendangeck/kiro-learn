...
The [Reconciliation Stage](/architecture/ingestion#reconciliation-stage) is the only caller that deletes memory records. When the judge model rules that a cluster of candidates and some subset of existing neighbors describe the same thing, the reconciler commits a new summary record **and deletes the merged originals** in the same transaction. There is no soft delete, no tombstone, no `deleted_at` column. The merged rows are gone.

The cascade is explicit. `deleteMemoryRecord(recordIds)` drops three things for every id, all inside one `db.transaction(...)` block:
...
All three `DELETE`s are explicit prepared statements. SQLite's FTS5 here is a plain virtual table (no content-linked `INSERT … INTO … (content)` rowid coupling), and there are no `ON DELETE` triggers in any migration — the FTS5 companion row is managed in application code the same way it is written in `putMemoryRecord`. A missing cascade statement would leak a ghost FTS row; the property tests catch that by asserting the FTS index is empty for every deleted id.

Because `DELETE` on a nonexistent row affects zero rows and does not raise, `deleteMemoryRecord` is idempotent. An id that was deleted in an earlier attempt is a silent no-op on retry, which is what lets a partially-crashed reconciliation run safely retry the same merge on the next ingestion trigger.

<Warning>
  **Reconciliation merges have no undo.** A `<merge>` decision deletes the original rows outright. Nothing in the database remembers that `record_id mr_abc123` used to exist, and nothing in the retrieval path can surface a merged-away record. Tune the [reconciliation thresholds](/architecture/ingestion#configuration) conservatively — a low intra-batch or neighbor similarity threshold paired with a lenient judge can silently collapse rows that should have stayed distinct, and the only recovery path is to re-learn from fresh events.
</Warning>

### No schema migration for reconciliation

Reconciliation adds no columns, no indexes, no tables, no triggers. The `memory_records` schema from migration `0005_memory_record_embedding` is the schema that reconciliation merges on top of. Upgrading to the new code runs zero new DDL.

The two storage surface changes are both code-level:

- A new `deleteMemoryRecord(recordIds)` method on the `StorageBackend` interface (and its SQLite implementation).
- A new `withTransaction(fn)` method that gives the reconciler a restricted handle for committing a summary-plus-deletes atomically.

Pre-existing rows keep working unchanged. Reconciliation only touches a row when it comes up as a neighbor of a new candidate cluster and the judge decides to merge it away — nothing proactively rewrites or migrates historical data.
...
<CardGroup cols={2}>
  ...
  <Card title="Ingestion" href="/architecture/ingestion">
    The pipeline that drives `putMemoryRecord` and `deleteMemoryRecord`
  </Card>
